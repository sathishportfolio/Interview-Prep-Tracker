// @ts-check
/**
 * sync/manualPull.js — Manual Pull button: force an on-demand refresh of the default bin from the
...
export async function manualPull() {
  ...
  if (!confirmAction("Pull the default bin from the cloud now? Matching local files will be overwritten with the cloud version.")) {
    return { ok: false };
  }
  const result = await bins.pullBinIntoLocalState(appState.sync.defaultBinId);
  if (!result.ok) {
